...
export async function fetchFundSnapshot(query: FundQuery, signal?: AbortSignal): Promise<FundSnapshot> {
  ...
  const response = await fetch(apiUrl(`/api/funds?${params}`), { signal, headers: { Accept: "application/json" } });
...
export async function fetchFundDetail(code: string, signal?: AbortSignal): Promise<FundDetail> {
  const response = await fetch(apiUrl(`/api/funds/${code}`), { signal, headers: { Accept: "application/json" } });
  const data = await response.json();
...
export async function fetchMarketOverview(signal?: AbortSignal): Promise<MarketOverview> {
  const response = await fetch(apiUrl("/api/market-overview"), { signal, headers: { Accept: "application/json" } });
  const data = await response.json();
